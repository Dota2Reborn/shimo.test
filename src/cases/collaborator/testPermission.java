...
//		click(list_addCollaborator_4);
	...
	// Boolean exist =
	// doesWebElementExist(By.xpath("//span[@data-test='change-table-sort-folder-priority']/following-sibling::span[1]"));
	...
	public void Add(String Emil) throws InterruptedException {
		try {
			sendKeys(input_addCollaborator, Emil);
			wait.until(ExpectedConditions.elementToBeClickable(b_addCollaborator_1_add));
			String name = getText(b_addCollaborator_1_add);
			if (!name.equals("添加")) {
	...
	// wait.until(ExpectedConditions.elementToBeClickable(button_addCollaborator));
	// String nn =
	// driver.findElement(By.xpath("//div[@class='sm-modal-body']//div[3]//div[1]//div[1]")).getAttribute("class");
	// List<WebElement> elements =
	// driver.findElements(By.xpath("//div[@class='sm-modal-body']//div[3]//div[1]//div[@class='"+nn+"']"));
	// int number = elements.size(); // 成员列表人数
	// //System.out.println(number+"个协作者");
	// for (int j = 2; j <= number; j++) {
	// String m =
	// driver.findElement(By.xpath("//div[@class='sm-modal-body']//div[3]//div[1]//div["+j+"]//span[1]//span[1]")).getText();
